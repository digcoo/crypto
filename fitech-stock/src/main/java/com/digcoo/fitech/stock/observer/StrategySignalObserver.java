package com.digcoo.fitech.stock.observer;
...
import com.digcoo.fitech.stock.core.OrderExecutionModule;
import com.digcoo.fitech.stock.core.PortfolioModule;
...
public class StrategySignalObserver implements CandlestickObserver {
    ...
    private static final String link_pref = "https://finance.sina.com.cn/realstock/company/%s/nc.shtml";
    ...
    public void onCandleUpdate(String symbol, CandlestickPeriod period, List<Candlestick> candlesticks) {
        ...
        for (TradeStrategy strategy : strategies) {
            ...
            try {
                ...
                if (Signal.NO_SIGNAL == signal || signal.getOrderSide() != OrderSide.BUY) {
//                log.info("({}) {}({}-{}) no signal"
//                        , candlestick0.getOpenTimeStr(), strategy.getStrategyType(), symbol, period);
                ...
                log.error("error generateSignal...symbol: {}, strategy: {}", symbol, strategy.getStrategyType(), ex);
            }
